...
local function startAutoLeft()
    ...
    autoLeftConnection = RunService.Heartbeat:Connect(function()
        ...
        if phase == 1 then
            ...
            moveToPosition(POSITION_L1)

            if (POSITION_L1 - hrp.Position).Magnitude < 2 then
                phase = 2
            end

        elseif phase == 2 then

            moveToPosition(POSITION_L2)

            if (POSITION_L2 - hrp.Position).Magnitude < 2 then
                autoLeft = false
                autoLeftConnection:Disconnect()
            end
...
local function startAutoRight()
    ...
    autoRightConnection = RunService.Heartbeat:Connect(function()
        ...
        if phase == 1 then
            ...
            moveToPosition(POSITION_R1)

            if (POSITION_R1 - hrp.Position).Magnitude < 2 then
                phase = 2
            end

        elseif phase == 2 then

            moveToPosition(POSITION_R2)

            if (POSITION_R2 - hrp.Position).Magnitude < 2 then
                autoRight = false
                autoRightConnection:Disconnect()
            end
